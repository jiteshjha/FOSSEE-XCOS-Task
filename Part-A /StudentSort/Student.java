...
public class Student implements Comparator<Student>{
  ...
  @Override
	public int compare(Student s1, Student s2) {
     // Compare total marks
	   int compareStudent = new Float(s1.getTotal()).compareTo(new Float(s2.getTotal()));

     // If total marks are same, compare language1 marks.
     if(compareStudent == 0) {
       compareStudent = new Float(s1.getLanguage1Marks()).compareTo(new Float(s2.getLanguage1Marks()));
     }
     // If language1 marks are same, compare language2 marks.
     if(compareStudent == 0) {
       compareStudent = new Float(s1.getLanguage2Marks()).compareTo(new Float(s2.getLanguage2Marks()));
     }
     // If language2 marks are same, compare math marks.
     if(compareStudent == 0) {
       compareStudent = new Float(s1.getMathMarks()).compareTo(new Float(s2.getMathMarks()));
     }
     // If math marks are same, compare science marks.
    if(compareStudent == 0) {
       compareStudent = new Float(s1.getScienceMarks()).compareTo(new Float(s2.getScienceMarks()));
    }
    // If science marks are same, compare environmental science marks.
    if(compareStudent == 0) {
       compareStudent = new Float(s1.getEnvironmentalScienceMarks()).compareTo(new Float(s2.getEnvironmentalScienceMarks()));
    }
